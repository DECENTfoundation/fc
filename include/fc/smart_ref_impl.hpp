...
namespace fc {
    ...
    namespace detail {
      ...
      struct insert_op {
        typedef decltype( *((A*)0) << *((std::remove_reference_t<U>*)0) ) type; 
      };
    ...
    smart_ref<T>::smart_ref( U&& u ) {
      impl = new (this) T( std::forward<U>(u) );
    }

    template<typename T>
    template<typename U,typename V>
    smart_ref<T>::smart_ref( U&& u, V&& v ) {
      impl = new T( std::forward<U>(u), std::forward<V>(v) );
    }
    template<typename T>
    template<typename U,typename V,typename X,typename Y>
    smart_ref<T>::smart_ref( U&& u, V&& v, X&& x, Y&&  y ) {
      impl = new T( std::forward<U>(u), std::forward<V>(v), std::forward<X>(x), std::forward<Y>(y) );
    }
