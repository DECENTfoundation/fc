      ...
      public:
         ...
         void remove( int udt_socket_id )
         {
             { synchronized(_read_promises_mutex)
                ...
                {
                   read_itr->second->set_exception( std::make_shared<unhandled_exception>( log_message(), std::make_exception_ptr( exception() ) ) );
             ...
             { synchronized(_write_promises_mutex)
                ...
                {
                   write_itr->second->set_exception( std::make_shared<unhandled_exception>( log_message(), std::make_exception_ptr( exception() ) ) );
